function door_loop(o)
    ...
    o.hitboxRadius = 0
    o.hitboxHeight = 0
    local starsNeeded = (o.oBehParams >> 24) or 0
    if o.oAction == 0 then
      -- if mario is close enough, set action to the custom open door action, 5
      if gMarioStates[0].numStars >= starsNeeded and  dist_between_objects(o, gMarioStates[0].marioObj) <= 400 then
          o.oAction = 5
      end
    end
...
function star_door_loop(o)
    ...
    if  m.numStars >= starsNeeded and  dist_between_objects(m.marioObj, o) <= 800 then
